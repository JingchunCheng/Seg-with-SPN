...
# generate objects fg-bg label for 1st frame image
class DAVIS1stFrmFgbgDataLayer(caffe.Layer):
    """
    ...
    def setup(self, bottom, top):
        ...
        if self.split == 'val':
           split_f         = '{}/ImageSets/2017/class/{}.txt'.format(self.davis_dir, self.split)
           self.classes    = open(split_f, 'r').read().splitlines()
           self.idx        = -1
        else:
           self.classes    = ['']
        ...
        if self.random and self.split == 'val':
            random.seed(self.seed)
            self.idx = random.randint(0, len(self.classes)-1)
    ...
    def reshape(self, bottom, top):
        ...
        while True:
            # pick next input
            if self.random and self.split == 'val' and self.idx == len(self.classes):
                self.idx = random.randint(0, len(self.classes)-1)
            elif self.random and self.split == 'val':
                self.idx += 1
                if self.idx == len(self.classes):
                    self.idx = 0
